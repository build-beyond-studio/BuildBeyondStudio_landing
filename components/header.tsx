        ...
        <div className={`nav-morph ${scrolled ? "scrolled" : ""}`}>
          ...
          <div className="flex items-center justify-between px-5 sm:px-6">
            ...
            <div className="anim-from-left flex items-center gap-2.5">
              ...
              />
              <span className="font-semibold text-[#111] text-sm sm:text-[15px] tracking-tight">
                Build Beyond Studio
              </span>
            </div>
            ...
            <nav className="hidden md:flex items-center gap-7">
              {NAV_LINKS.map(({ href, label }) => (
                <a
                  key={href}
                  href={href}
                  className="nav-item nav-link text-[#444] hover:text-[#111] text-[13px] font-medium transition-colors duration-200"
                >
                  {label}
                </a>
              ))}
            ...
            <button
              className={`anim-from-right md:hidden flex flex-col justify-center items-center gap-[5.5px] w-9 h-9 rounded-xl hover:bg-black/5 transition-colors duration-200 cursor-pointer ${
                isMenuOpen ? "ham-open" : ""
              }`}
          ...
          <div className={`mobile-menu md:hidden ${isMenuOpen ? "open" : ""}`}>
            <div className="mobile-menu-inner">
              <nav className="flex flex-col px-5 pb-4 pt-3 gap-0.5 border-t border-black/5 mt-2">
                {NAV_LINKS.map(({ href, label }) => (
                  <a
                    key={href}
                    href={href}
                    onClick={() => setIsMenuOpen(false)}
                    className="mob-link text-[#444] hover:text-[#111] text-sm font-medium py-2.5 border-b border-black/[0.06] last:border-0 transition-colors duration-200"
                  >
                    {label}
                  </a>
                ))}
